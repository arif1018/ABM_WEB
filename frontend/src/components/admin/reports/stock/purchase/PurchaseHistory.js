...
import { getData } from '../../../../../features/reports/stock/purchase/purchaseHistorySlice'
...
export default function PurchaseHistory() {
    ...
    const { phmData, isLoading } = useSelector((state)=>state.PHMD)
    ...
    <DBMenu />
        <Container component="main" sx={{mt:5}}>
            <Box component="form" onSubmit={handleSubmit} noValidate sx={{ mt: 1 }}>
                <Typography fontWeight={'bolder'} fontSize={30} align={'center'}>Purchase History</Typography>
            ...
            <Grid container spacing={2} direction="row" alignItems="center">
                ...
                <Grid item xs={12}>
                    ...
                    <Document title={`Purchase_History_${dayjs(Date.now()).format('DDMMYYYY_hh:mmA')}`}>
                        ...
                        {isLoading ? (<View><Text style={{fontSize:'30px', fontWeight:'bold', fontFamily:'TimesNewRoman'}}>Please Wait Data Loading</Text></View>) : (
                            phmData.map( item =>(
                                <View style={stylesTR.row} key={Math.random.toString()}>
                                    ...
                                    <Text style={stylesTR.trrate}>{Math.round(item.RR)}</Text>
                                    <Text style={stylesTR.tramount}>{Math.round(item.RR * item.Quantity).toLocaleString("en-US")}</Text>
