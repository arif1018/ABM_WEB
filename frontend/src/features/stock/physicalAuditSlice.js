...
import partiesService from './physicalAuditService'
...
export const dataForBarCode = createAsyncThunk('PA_BDetail/get', async(PDRONO_UserID, thunkAPI)=>{
    try {
...
export const dataForBarCodeOld = createAsyncThunk('PA_BDetailOLD/get', async(OldBarCode, thunkAPI)=>{
    try {
        return await partiesService.dataForBarCodeOld(OldBarCode)
    } catch (error) {
        const message = (error.response && error.response.data && error.response.data.message) || error.message || error.toString()
        return thunkAPI.rejectWithValue(message)
    }
...
export const dataForDataTable = createAsyncThunk('PA_G_Data/get', async(UserID, thunkAPI)=>{
    try {
...
export const addNewRecord = createAsyncThunk('PA_A_N_R/post', async(userData, thunkAPI)=>{
    try {
...
export const stockSlice = createSlice({
    ...
    extraReducers:(builder)=>{
        builder
            ...
            .addCase(dataForBarCodeOld.pending, (state)=>{
                state.isLoadingBarCodeDetail = true
            })
            .addCase(dataForBarCodeOld.fulfilled, (state, action)=>{
                state.isLoadingBarCodeDetail = false
                state.isSuccess = true
                state.BarCodeDetail = action.payload
            })
            .addCase(dataForBarCodeOld.rejected, (state, action)=>{
                state.isLoadingBarCodeDetail = false
                state.isError = true
                state.message = action.payload
                state.BarCodeDetail = []
